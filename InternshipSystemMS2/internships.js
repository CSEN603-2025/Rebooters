...
// Use localStorage if available, otherwise set defaults
let internships = JSON.parse(localStorage.getItem('internships'));
if (!internships || !Array.isArray(internships) || internships.length === 0) {
    // If localStorage is empty or invalid, use the default Internships array
    internships = Internships;
    localStorage.setItem('internships', JSON.stringify(Internships));
}

console.log('Final internships array:', internships);
...
function renderInternships() {
    ...
    let filtered = internships.filter(i => {
        ...
        const matchesDuration = !duration || i.duration === parseInt(duration, 10);
    ...
    filtered.forEach(i => {
        console.log('Rendering internship:', i);
...
// Move viewDetails function definition to global scope
...
function loadMajorVideo() {
    ...
    const userMajor = localStorage.getItem('userMajor') ||"Business Informatics";
